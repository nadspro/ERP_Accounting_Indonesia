...
(function(g, v) {
    var o = g.each, s = g.extend, p = g.merge, w = g.map, j = g.pick, q = g.pInt, i = g.getOptions().plotOptions, k = g.seriesTypes, t = g.Series, m = function() {
            ...
            if (this.center)
                this.transA = this.isCircular ? (this.endAngleRad - this.startAngleRad) / (this.max - this.min + (this.closestPointRange || 0) || 1) : this.center[2] / 2 / (this.max - this.min || 1), this.minPixelPadding = this.transA * ((this.pointRange || 0) / 2)
    ...
    r.init = function(a) {
        return function(b, c) {
            ...
            if (d) {
                ...
                s(this, u), this.defaultRadialOptions = (f = h) ? this.defaultRadialXOptions : this.defaultRadialYOptions;
            a.apply(this, arguments);
            if (d || e)
                d = this.options, this.startAngleRad = (d.startAngle - 90) * Math.PI / 180, this.endAngleRad = (d.endAngle - 90) * Math.PI / 180, this.offset = d.offset || 0, this.isCircular = f
        }
    }(r.init);
