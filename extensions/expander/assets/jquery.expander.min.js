...
(function(c) {
    ...
    c.fn.expander = function(l) {
        ...
        var v = c.extend({},
                c.expander.defaults, l), M = /^<(?:area|br|col|embed|hr|img|input|link|meta|param).*>$/i, K = v.wordEnd || /(&(?:[^;]+;)?|[a-zA-Z\u00C0-\u0100]+)$/, B = /<\/?(\w+)[^>]*>/g, C = /<(\w+)[^>]*>/g, D = /<\/(\w+)>/g, x = /(<\/[^>]+>)\s*$/, N = /^<[^>]+>.?/, A;
        l = {init: function() {
                this.each(function() {
                    var a, d, e, h, m, i, n, o, w, E = [], s = [], q = {}, r = this, f = c(this), F = c([]), b = c.extend({}, v, f.data("expander") || c.meta && f.data() || {});
                    ...
                    if (!c.data(this, "expanderInit")) {
                        ...
                        for (a = 0; a < d; a++)
                            m[a] = m[a].replace(D, "$1");
                        c.each(h, function(j, k) {
                            var G = k.replace(C, "$1"), H = c.inArray(G, m);
                            if (H === -1) {
                                E.push(k);
                                s.push("</" + G + ">")
                            } else
                        ...
                        b.collapseEffect === "slideUp" && b.expandEffect !== "slideDown" || f.is(":hidden") ? o.css({display: "none"}) : o[b.collapseEffect](0);
                        F = f.find("div." + b.summaryClass);
                        w.find("a").unbind("click.expander").bind("click.expander",
                                function(j) {
                                    j.preventDefault();
                                    w.hide();
                                    F.hide();
