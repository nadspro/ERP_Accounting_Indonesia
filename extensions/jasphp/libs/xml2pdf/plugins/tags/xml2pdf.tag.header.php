    ...
    private function _renderComplexHeader() {
        ...
        foreach ($this->elements as $element) {
            ...
            if (is_a($element, 'xml2pdf_tag_paragraph')) {
                ...
                $this->pdf->MultiCell($width, $lineHeight, $element->content, $border, $textAlign, $fill);
                //calcul de la hauteur max du header
                if ($this->_height < $this->pdf->GetY()) {
                    $this->_height = $this->pdf->GetY();
                }
                //r�initialisation des valeurs pour la suite
                $this->_resetValues();
...
//                    $top += $this->pdf->GetY();
//                }
                $this->pdf->Image((string) $element->file, $left, $top, $element->width, $element->height, $element->type);
            }
        }
