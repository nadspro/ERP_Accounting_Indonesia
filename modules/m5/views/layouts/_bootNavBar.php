...
if (!Yii::app()->user->isGuest) {
	...
	if (!Yii::app()->cache->get('m5_hierarchy1'.Yii::app()->user->id)) {
		if (Yii::app()->user->name == 'admin') {
			$m5_hierarchy=menu::model()->findAll(array('condition'=>'parent_id = \'0\' AND (name = \'m5\' OR name = \'m0\') ','order'=>'sort'));
		} else {
			...
			//$m5_hierarchy=menu::model()->findAllBySql('SELECT a.id FROM s_module a
			//		LEFT JOIN s_user_module b ON a.id = b.s_module_id
			//		WHERE a.parent_id = "0"
			//		AND b.s_user_id = '.Yii::app()->user->id .' order by sort');
			$m5_hierarchy=menu::model()->cache(3600,$dependency)->findAll($criteria);
		}
		Yii::app()->cache->set('m5_hierarchy1'.Yii::app()->user->id,$m5_hierarchy,86400,$dependency);
	} else
		$m5_hierarchy=Yii::app()->cache->get('m5_hierarchy1'.Yii::app()->user->id);
	
	if (!Yii::app()->cache->get('m5_hierarchy2'.Yii::app()->user->id)) {
		foreach ($m5_hierarchy as $Hierarchy){
			$models = menu::model()->findByPk($Hierarchy->id);
			$items[] = $models->getListed();
		}		
		Yii::app()->cache->set('m5_hierarchy2'.Yii::app()->user->id,$items,86400,$dependency);
	} else
		$items=Yii::app()->cache->get('m5_hierarchy2'.Yii::app()->user->id);
