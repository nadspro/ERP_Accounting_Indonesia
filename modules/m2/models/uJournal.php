    ...
    public static function getTopUpdated($mid = null) {
        ...
        foreach ($models as $model) {
            if ($model->module_id == 1) {
                $returnarray[] = array('id' => $model->system_ref, 'label' => $model->system_ref, 'icon' => 'list-alt', 'url' => array('/m2/uJournal/view', 'id' => $model->id));
            } else {
                $returnarray[] = array('id' => $model->system_ref, 'label' => $model->system_ref, 'icon' => 'list-alt', 'url' => array('/m2/mCashbank/view', 'id' => $model->id));
            }
        }
