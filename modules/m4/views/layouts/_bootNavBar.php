...
if (!Yii::app()->user->isGuest) {
	...
	if (!Yii::app()->cache->get('m4_hierarchy1'.Yii::app()->user->id)) {
		if (Yii::app()->user->name == 'admin') {
			$m4_hierarchy=menu::model()->findAll(array('condition'=>'parent_id = \'0\' AND (name = \'m4\' OR name = \'m0\') ','order'=>'sort'));
		} else {
			...
			//$m4_hierarchy=menu::model()->findAllBySql('SELECT a.id FROM s_module a
			//		LEFT JOIN s_user_module b ON a.id = b.s_module_id
			//		WHERE a.parent_id = "0"
			//		AND b.s_user_id = '.Yii::app()->user->id .' order by sort');
			$m4_hierarchy=menu::model()->cache(3600,$dependency)->findAll($criteria);
		}
		Yii::app()->cache->set('m4_hierarchy1'.Yii::app()->user->id,$m4_hierarchy,86400,$dependency);
	} else
		$m4_hierarchy=Yii::app()->cache->get('m4_hierarchy1'.Yii::app()->user->id);
	
	if (!Yii::app()->cache->get('m4_hierarchy2'.Yii::app()->user->id)) {
		foreach ($m4_hierarchy as $Hierarchy){
			$models = menu::model()->findByPk($Hierarchy->id);
			$items[] = $models->getListed();
		}		
		Yii::app()->cache->set('m4_hierarchy2'.Yii::app()->user->id,$items,86400,$dependency);
	} else
		$items=Yii::app()->cache->get('m4_hierarchy2'.Yii::app()->user->id);
