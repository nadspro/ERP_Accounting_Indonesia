...
if (!Yii::app()->user->isGuest) {
	...
	if (!Yii::app()->cache->get('m3_hierarchy1'.Yii::app()->user->id)) {
		if (Yii::app()->user->name == 'admin') {
			$m3_hierarchy=menu::model()->findAll(array('condition'=>'parent_id = \'0\' AND (name = \'m3\' OR name = \'m0\') ','order'=>'sort'));
		} else {
			...
			//$m3_hierarchy=menu::model()->findAllBySql('SELECT a.id FROM s_module a
			//		LEFT JOIN s_user_module b ON a.id = b.s_module_id
			//		WHERE a.parent_id = "0"
			//		AND b.s_user_id = '.Yii::app()->user->id .' order by sort');
			$m3_hierarchy=menu::model()->cache(3600,$dependency)->findAll($criteria);
		}
		Yii::app()->cache->set('m3_hierarchy1'.Yii::app()->user->id,$m3_hierarchy,86400,$dependency);
	} else
		$m3_hierarchy=Yii::app()->cache->get('m3_hierarchy1'.Yii::app()->user->id);
	
	if (!Yii::app()->cache->get('m3_hierarchy2'.Yii::app()->user->id)) {
		foreach ($m3_hierarchy as $Hierarchy){
			$models = menu::model()->findByPk($Hierarchy->id);
			$items[] = $models->getListed();
		}		
		Yii::app()->cache->set('m3_hierarchy2'.Yii::app()->user->id,$items,86400,$dependency);
	} else
		$items=Yii::app()->cache->get('m3_hierarchy2'.Yii::app()->user->id);
